...
public class ReportService : IReportService
{
    ...
    public async Task<LeaseContractsReport> GetLeaseContractsReportAsync(
        bool? isActive = null,
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        // Filter by active status (Active/Expiring vs Expired/Terminated/Renewed)
        if (isActive.HasValue)
        {
            if (isActive.Value)
                query = query.Where(lc => lc.Status == LeaseStatus.Active || lc.Status == LeaseStatus.Expiring);
            else
                query = query.Where(lc => lc.Status == LeaseStatus.Expired || lc.Status == LeaseStatus.Terminated || lc.Status == LeaseStatus.Renewed);
        }
        ...
        {
            var cutoffDate = DateTime.UtcNow.AddDays(daysUntilExpiration.Value);
            contracts = contracts.Where(lc => lc.EndDate <= cutoffDate).ToList();
        }

        var totalContracts = contracts.Count;
        var activeContracts = contracts.Count(c => c.Status == LeaseStatus.Active || c.Status == LeaseStatus.Expiring);
        var expiringContracts = contracts.Count(c => c.Status == LeaseStatus.Expiring || ((c.EndDate - DateTime.UtcNow).TotalDays <= 90 && c.Status == LeaseStatus.Active));

        var contractDtos = contracts.Select(c => new LeaseContractDto(
            c.Id,
            c.AssetId,
            c.ContractNumber,
            c.Vendor,
        ...
        return new LeaseContractsReport(totalContracts, activeContracts, expiringContracts, contractDtos);
    }

    public async Task<byte[]> ExportLeaseContractsCsvAsync(
        bool? isActive = null,
        CancellationToken cancellationToken = default)
    {
        var report = await GetLeaseContractsReportAsync(isActive, null, cancellationToken);

        var csv = new StringBuilder();
        csv.AppendLine("Contract Number,Vendor,Asset ID,Start Date,End Date,Days Until Expiration,Monthly Rate,Total Value,Status,Notes");

        foreach (var contract in report.Contracts)
        {
            var daysUntil = (contract.EndDate - DateTime.UtcNow).Days;
            csv.AppendLine($"\"{contract.ContractNumber}\",\"{contract.Vendor}\",\"{contract.AssetId}\",\"{contract.StartDate:yyyy-MM-dd}\",\"{contract.EndDate:yyyy-MM-dd}\",\"{daysUntil}\",\"{contract.MonthlyRate:F2}\",\"{contract.TotalValue:F2}\",\"{contract.Status}\",\"{contract.Notes}\"");
        }
